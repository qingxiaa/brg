#'@title Batch effect Reduction of mIcroarray data with Dependent samples usinG an Empirical Bayes approach (BRIDGE) for power assessment
...
#' @noRd
BRIDGE_pwr <- function(infile, SubjectID, batch, time, data, imputation = FALSE, optimization = FALSE){
  #set.seed(2020)
  ...
  Obs <- c(1:ObsSize)
  # Extract labels of batch and time points information
  ...
  if (N1 == N2){
    ...
    cat("[BRIDGE] found", ObsSize,"total observations", N1, "subjects in time 1 and ",
        N2, "subjects in time 2,", J, "features\n", M, "bridging samples")
  }
  ...
  mu01j_h <- apply(Y1_t1, 2, mean)
  sigma01j_h <- apply(t(t(Y1_t1) - mu01j_h), 2, sd)
  phi2j_h <- apply(Y2_t1_M, 2, mean) - apply(Y1_t1_M, 2, mean)
  delta2j_h <- apply(t(t(Y2_t1_M) - apply(Y2_t1_M, 2, mean)), 2, sd)/sigma01j_h
  sigma02j_h <- apply(t(t(Y2_t2) - apply(Y2_t2, 2, mean)), 2, sd)/delta2j_h
  mu02j_h <- apply(t(t(Y2_t2) - phi2j_h), 2, mean)
  
  
  Z1t1 <- matrix(NA, nrow=N1, ncol=J)
  Z2t1_M <- matrix(NA, nrow=M, ncol=J)
  Z2t2 <- matrix(NA, nrow=N2, ncol=J)
  Z1t1_M <- matrix(NA, nrow=M, ncol=J)
  
  # Normalize data
  cat("[BRIDGE] standardizing data across features\n")
  for (j in 1:J){
    Z1t1[, j] <- (Y1_t1[, j] - mu01j_h[j])/sigma01j_h[j]
    Z2t2[, j] <- (Y2_t2[, j] - mu02j_h[j] - phi2j_h[j])/sigma02j_h[j] + phi2j_h[j]
    Z1t1_M[, j] <- (Y1_t1_M[, j] - mu01j_h[j])/sigma01j_h[j]
    Z2t1_M[, j] <- (Y2_t1_M[, j] - mu01j_h[j] - phi2j_h[j])/sigma01j_h[j] + phi2j_h[j]
  }
  
  ########## Step 2: Empirical Bayes Estimate of Hyperprior and LS parameters########
  
  # 1) Empirical Hyperprior estimates from linear regression
  ...
  beta_EB <- NULL
  sigma_EB <- NULL
  out <- matrix(list(), nrow= J, ncol=2)
  for (j in 1:J){
    Gibbs <- list()
    x <- Z1t1_M[, j]
    y <- Z2t1_M[, j]
    Gibbs <- GibbsSampler_pwr(y, x, beta0, precS, lambda0, theta0)
    out[[j, 1]] <- Gibbs[[1]]
    out[[j, 2]] <- Gibbs[[2]]
    # Empirical Bayes Estimates for method 2
    sigma_EB[j] <- mean(sqrt(out[[j, 2]]))
    alpha_EB[j] <- apply(out[[j, 1]], 2, mean)[1]
    beta_EB[j] <- apply(out[[j, 1]], 2, mean)[2]
  }
  ...
  if ( (imputation == FALSE & optimization == FALSE) | (optimization == TRUE & mean(delta2_EB) > 1)){
    ...
    adj_y1t1 <- matrix(NA, nrow = N1, ncol = J)
    adj_y2t2 <- matrix(NA, nrow = N2, ncol = J)
    adj_y2t1_M <- matrix(NA, nrow = M, ncol = J)
    for(j in 1:J){
      adj_y1t1[, j] <- BE_Adjustment(0, 1, Z1t1[, j], mu01j_h[j], sigma01j_h[j])
      adj_y2t1_M[, j] <- BE_Adjustment(phi2_EB[j], delta2_EB[j], Z2t1_M[, j], mu01j_h[j], sigma01j_h[j])
      adj_y2t2[, j] <- BE_Adjustment(phi2_EB[j], delta2_EB[j], Z2t2[, j], mu02j_h[j], sigma02j_h[j])
    }
    
    adj_DataMat <- rbind(adj_y1t1,adj_y2t1_M,adj_y2t2)
    factor.df <- rbind(data[Y1t1_Obs,], data[bridge_Obs,][SubjectID[bridge_Obs] %in% bridge_id, ],data[Y2t2_Obs,])
    ...
    Z2t1_P <- pred(alpha_EB, beta_EB, bridge_ind, Z1t1, Z2t1_M, J, M, N1)
    Y2t1_P <- matrix(NA, N1, J)
    for (j in 1:J){
      Y2t1_P[, j] <- (Z2t1_P[, j] - phi2_EB[j]) * sigma01j_h[j] + phi2_EB[j] + mu01j_h[j]  
    }
    adj_DataMat <- rbind(Y1_t1,Y2t1_P,Y2_t2)
    labs <- rep(batch2,N1)
    labs[SubjectID[Y1t1_Obs] %in% bridge_id] = "Imputed"
    imputed.label <- labs
    new.batch <- c(rep(batch1, N1),imputed.label,rep(batch2,N2))
    factor.df <- rbind(data[Y1t1_Obs,],data[Y1t1_Obs,],data[Y2t2_Obs,])
    factor.df$batch <- new.batch
    AdjustedData <- t(adj_DataMat)
  }
