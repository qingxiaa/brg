...
#' @import shiny ggplot2 gridExtra BiocManager shinyBS sva longCombat
#'@rdname pwrBRIDGE_shiny
  ...
  server <- function(session,input, output) {
    Inference_comparison <- function(para, setSeeds, method = 1, iter){
      ...
      for (i in 1:iter){
        ...
        # Simulate data
        simulation.out <- do.call(SimulateData_pwr, para)
        # Data N*J
    ...
    create.plot <- reactive({
      ...
      p1<-ggplot(create.table()$out, aes(x = create.table()$out$M, y=create.table()$out$TypeI, 
                                         group = create.table()$out$methods, colour = create.table()$out$methods)) +
        geom_line(size=1) +
        geom_point()+
        # geom_errorbar(aes(ymin=TypeI-sd, ymax=TypeI+sd), width=.5,position=position_dodge(0.05))+
        theme_classic() +
        geom_text(data = create.table()$out, aes(label=round(create.table()$out$TypeI,3), y= create.table()$out$TypeI+0.03),
                  position = position_dodge(3),size = 4)+
        labs(title="Type I error rate as bridging sample size increases")
      p2<-ggplot(create.table()$out, aes(x = create.table()$out$M, y=create.table()$out$Power, 
                                         group = create.table()$out$methods, colour = create.table()$out$methods)) +
        ...
        # geom_errorbar(aes(ymin=TypeI-sd, ymax=TypeI+sd), width=.5,position=position_dodge(0.05))+
        theme_classic() +
        geom_text(data = create.table()$out, aes(label=round(create.table()$out$Power,3), y= create.table()$out$Power+0.01),
                  position = position_dodge(3),size = 4)+
